...
import { fetchProductsBySeller } from "../../apis/productApi";
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const result = await fetchProductsBySeller(uid);
        ...
        if (result.status && Array.isArray(result.data)) {
          const foodsOnly = result.data.filter((item) => item.type === "foods");
          setFoodItems(foodsOnly);
        } else {
      ...
      <div className="flex-1 p-8">
        ...
        {!loading && !error && foodItems.length === 0 && (
          <p className="text-center text-gray-500">
            No food items available. Start by adding one!
          </p>
        )}
